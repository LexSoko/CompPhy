...
# -*- coding: utf-8 -*-
"""
Execution script for complete task 2 of the computational physics assignment 1
Since the subtasks 2.b, 2.c and 2.d are so interwoven, the script isn't split
up into these subsections but 
...
dt_list = [0.001] * n_cond  # list of timesteps
...
fig.suptitle("Exercise 2", fontsize=20)
fig.subplots_adjust(
...
for i_cond in tqdm(range(0, n_cond), desc="condition-loop"):
    # plotting setup
    ax_txt = fig.add_subplot(5, n_cond, i_cond + 1)
    ax_theta = fig.add_subplot(5, n_cond, i_cond + 1 + n_cond)
    ax_p = fig.add_subplot(5, n_cond, i_cond + 1 + n_cond * 2)
    ax_E = fig.add_subplot(5, n_cond, i_cond + 1 + n_cond * 3)
    ax_poincare = fig.add_subplot(5, n_cond, i_cond + 1 + n_cond * 4)
    ...
    ############################### Task 2.b #######################################
    # execute the explicit runge-kutta 4th order
    t_list[i_cond], y_list[i_cond] = rk.rk4_method(
    ...
    # plot the angles
    ax_theta.plot(
        t_list[i_cond][: n_tmax_timeplots[i_cond]],
        y_list[i_cond][i["theta_1"], : n_tmax_timeplots[i_cond]],
        label=r"$\theta_1$",
        color="red",
    )
    ax_theta.plot(
        t_list[i_cond][: n_tmax_timeplots[i_cond]],
        y_list[i_cond][i["theta_2"], : n_tmax_timeplots[i_cond]],
        label=r"$\theta_2$",
        color="blue",
    )
    ax_theta.legend()
    ...
    ax_p.plot(
        t_list[i_cond][: n_tmax_timeplots[i_cond]],
        y_list[i_cond][i["p_1"], : n_tmax_timeplots[i_cond]],
        label=r"$p_1$",
        color="red",
    )
    ax_p.plot(
        t_list[i_cond][: n_tmax_timeplots[i_cond]],
        y_list[i_cond][i["p_2"], : n_tmax_timeplots[i_cond]],
        label=r"$p_2$",
        color="blue",
    )
    ax_p.legend()
    ...
    ax_E.plot(
        t_list[i_cond][: n_tmax_timeplots[i_cond]],
        E_list[i_cond][: n_tmax_timeplots[i_cond]],
        label=r"$E(t)$",
        color="green",
    )
    ax_E.legend()
...
fig.savefig("./Ex1/plots/plot_ale.pdf", dpi="figure")
